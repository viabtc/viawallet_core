...
Data decode(const string& val) {
    ...
    for (size_t i=0; i<encoded_size; i += 4) {
        bool base64Flag = is_base64(encoded_string[i]);
        if (!base64Flag) {
            throw std::invalid_argument("Error while Base64 decode");
        }
        // Get values for each group of four base 64 characters
        uint8_t b4[4];
        b4[0] = static_cast<uint8_t>((encoded_string[i+0] <= 'z') ? from_base64[encoded_string[i+0]] : 0xff);
        b4[1] = static_cast<uint8_t>((encoded_string[i+1] <= 'z') ? from_base64[encoded_string[i+1]] : 0xff);
        b4[2] = static_cast<uint8_t>((encoded_string[i+2] <= 'z') ? from_base64[encoded_string[i+2]] : 0xff);
        b4[3] = static_cast<uint8_t>((encoded_string[i+3] <= 'z') ? from_base64[encoded_string[i+3]] : 0xff);

        // Transform into a group of three bytes
        uint8_t b3[3];
        b3[0] = static_cast<uint8_t>(((b4[0] & 0x3f) << 2) + ((b4[1] & 0x30) >> 4));
        b3[1] = static_cast<uint8_t>(((b4[1] & 0x0f) << 4) + ((b4[2] & 0x3c) >> 2));
        b3[2] = static_cast<uint8_t>(((b4[2] & 0x03) << 6) + ((b4[3] & 0x3f) >> 0));
...
std::string encode(const uint8_t* buf, size_t bufLen) {
    ...
    for (unsigned int i=0; i<ret_size/4; ++i) {
        ...
        char b4[4];
        b4[0] = ((b3[0] & 0xfc) >> 2);
        b4[1] = static_cast<char>(((b3[0] & 0x03) << 4) + ((b3[1] & 0xf0) >> 4));
        b4[2] = static_cast<char>(((b3[1] & 0x0f) << 2) + ((b3[2] & 0xc0) >> 6));
        b4[3] = static_cast<char>((b3[2] & 0x3f) << 0);
