...
Data RLP::encode(const Transaction& transaction) noexcept {
    ...
    append(encoded, encode(transaction.storageLimit));
    append(encoded, encode(transaction.epochHeight));
    append(encoded, encode(transaction.chainId));
    append(encoded, encode(transaction.payload));

    const Data& vector = encodeList(encoded);

    auto encodedAll = Data();

    append(encodedAll, vector);
    append(encodedAll, encode(transaction.v));
    append(encodedAll, encode(transaction.r));
    append(encodedAll, encode(transaction.s));

    return encodeList(encodedAll);
}
...
Data RLP::decodeRawTransaction(const Data& data) {
    ...
    auto result = json {
        ...
        {"gasLimit", hexEncoded(decoded[2])},
        {"to", hexEncoded(decoded[3])},
        {"amount", hexEncoded(decoded[4])},
        {"storageLimit", hexEncoded(decoded[5])},
        {"epochHeight", hexEncoded(decoded[6])},
        {"chainId", hexEncoded(decoded[7])},
        {"payload", hexEncoded(decoded[8])},
        {"v", hexEncoded(decoded[9])},
        {"r", hexEncoded(decoded[10])},
        {"s", hexEncoded(decoded[11])},
    }.dump();
