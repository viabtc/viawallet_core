...
static const std::string cashHRP = "simpleledger";
...
bool Address::isValid(const std::string& string) {
    ...
    size_t dataLen;
    if (cash_decode(hrp.data(), data.data(), &dataLen, withPrefix.c_str()) == 0 || dataLen != Address::size) {
        return false;
    }
...
Address::Address(const std::string& string) {
    ...
    if (!success || std::strncmp(hrp.data(), cashHRP.c_str(), std::min(cashHRP.size(), maxHRPSize)) != 0 || dataLen != Address::size) {
...
Address::Address(const PublicKey& publicKey) {
    if (publicKey.type != TWPublicKeyTypeSECP256k1) {
        throw std::invalid_argument("SimpleLedger address needs a compressed SECP256k1 public key.");
    }
    ...
    assert(success && outlen == Address::size);
}

std::string Address::string() const {
    std::array<char, 129> result;
    cash_encode(result.data(), cashHRP.c_str(), bytes.data(), Address::size);
    return result.data();
}

TW::Bitcoin::Address Address::legacyAddress() const {
    std::vector<uint8_t> result(TW::Bitcoin::Address::size);
    size_t outlen = 0;
    cash_data_to_addr(result.data(), &outlen, bytes.data(), Address::size);
    assert(outlen == 21 && "Invalid length");
    if (result[0] == p2khVersion) {
        result[0] = TW::p2pkhPrefix(TWCoinTypeSimpleLedger);
    } else if (result[0] == p2shVersion) {
        result[0] = TW::p2shPrefix(TWCoinTypeSimpleLedger);
    }
    return TW::Bitcoin::Address(result);
}
