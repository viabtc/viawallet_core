...
bool TWCosmosAddressIsValid(enum TWCoinType coin, TWString *_Nonnull string) {
    auto s = reinterpret_cast<const std::string*>(string);
    return Address::isValid(coin, *s);
}
...
struct TWCosmosAddress *_Nullable TWCosmosAddressCreateWithHRP(TWString *_Nonnull hrp, struct TWPublicKey *_Nonnull publicKey) {
    auto s = reinterpret_cast<const std::string*>(hrp);
    return new TWCosmosAddress{ Address(*s, publicKey->impl) };
}
