...
public final class StoredKey {
    ...
    public static func importHDWalletSingle(mnemonic: String, name: String, password: Data, coin: CoinType) -> StoredKey? {
        let mnemonicString = TWStringCreateWithNSString(mnemonic)
        defer {
            TWStringDelete(mnemonicString)
        }
        ...
        guard let value = TWStoredKeyImportHDWalletSingle(mnemonicString, nameString, passwordData, TWCoinType(rawValue: coin.rawValue)) else {
            return nil
        }
        return StoredKey(rawValue: value)
    }
    ...
    public var isPrivateKey: Bool {
        return TWStoredKeyIsPrivateKey(rawValue)
    }

    /// Whether this key is a mnemonic phrase for a HD wallet with only one chain.
    public var isMnemonicSingle: Bool {
        return TWStoredKeyIsMnemonicSingle(rawValue)
    }
