...
TEST(NervosCKBSigner, Plan) {
    auto input = Proto::SigningInput();
    auto transfer = input.mutable_transfer();
    transfer->set_amount(6200000000);
    transfer->set_fee_rate(1000);
    transfer->set_to("ckb1qyqrzv0lvhr6r4ljm9utx3l0ccll29usgfnsw45jkq");
    transfer->set_use_max_amount(false);
    auto cell = transfer->add_cells();
    cell->set_since(0);
    cell->set_capacity(19000000000);
    ...
    EXPECT_EQ(plan.transfer().fee(), 464);
    EXPECT_EQ(plan.transfer().amount(), 6200000000);
    EXPECT_EQ(plan.transfer().change(), 12799999536);
    EXPECT_EQ(plan.transfer().available_amount(), 19000000000);
}
